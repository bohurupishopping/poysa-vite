...
export default function CustomerTransactions() {
  ...
  const loadTransactions = useCallback(async () => {
    ...
    try {
      ...
      const [transactionsData, balance, customerData] = await Promise.all([
        ...
        CustomerService.getCustomerBalance(customerId),
  ...
  const loadMoreTransactions = useCallback(async () => {
    if (!customerId || isLoadingMore || !hasMoreData) return;
    
    try {
      setIsLoadingMore(true);
      
      const nextPage = currentPage + 1;
      const transactionsData = await CustomerService.getCustomerTransactions(customerId, {
        startDate: dateRange.startDate,
        endDate: dateRange.endDate,
        page: nextPage,
        limit: pageSize
  ...
  }, [customerId, currentPage, dateRange, isLoadingMore, hasMoreData]);
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      const allTransactions = await CustomerService.getCustomerTransactions(customerId, {
        startDate: dateRange.startDate,
        endDate: dateRange.endDate,
        page: 1,
        limit: 10000
      ...
      const filename = `Customer_Statement_${customer.name}_${dateRange.startDate.toISOString().split('T')[0]}_to_${dateRange.endDate.toISOString().split('T')[0]}.pdf`;
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm font-medium" style={{ color: UI_CONSTANTS.colors.textSecondary }}>
                    {currentBalance >= 0 ? 'Amount Receivable' : 'Amount Payable'}
                  </p>
                  <p className="text-2xl font-bold" style={{ color: currentBalance >= 0 ? UI_CONSTANTS.colors.success : UI_CONSTANTS.colors.danger }}>
